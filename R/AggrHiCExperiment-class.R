...
#' @param flanking.bins Number of bins on each flank of the bins containing 
#'   input targets.
...
AggrHiCExperiment <- function(
    ...
    if (is(targets, 'GRanges')) {
        ...
        targets <- GenomicRanges::resize(
            targets, fix = 'center', width = resolution*{2*flanking.bins}
        )
        ...
        an1 <- ans[[1]]
        an1 <- resize(an1, fix = 'center', width = resolution*{2*flanking.bins})
        an2 <- ans[[2]]
        an2 <- resize(an2, fix = 'center', width = resolution*{2*flanking.bins})
