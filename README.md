...
`HiCExperiment` S4 class is built on pre-existing Bioconductor classes, namely `InteractionSet` and `ContactMatrix` (`Lun, Perry & Ing-Simmons, F1000Research 2016`), and leverages them to import on-disk Hi-C matrix files.
...
Additionally, we provide a docker image with HiCExperiment and related packages pre-installed and ready-to-go:
