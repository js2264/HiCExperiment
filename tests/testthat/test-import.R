test_that("imports work", {
    ...
    expect_s4_class(import(mcool_path, format = 'mcool'), 'HiCExperiment')
    expect_equal(
    ...
    expect_s4_class(import(hcf, format = 'cool'), 'HiCExperiment')
    expect_equal(
        resolution(import(hcf, format = 'mcool', resolution = 8000)), 
        8000
    )
